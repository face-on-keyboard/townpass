...
class HealthService extends GetxService {
  ...
  Future<bool> _ensureAuthorized() async {
    if (!isSupportedPlatform || _healthFactory == null) {
      return false;
    }
    ...
    bool hasPermissions =
        await _healthFactory!.hasPermissions(_dataTypes, permissions: permissions) ?? false;

    if (!hasPermissions) {
      hasPermissions =
          await _healthFactory!.requestAuthorization(_dataTypes, permissions: permissions);
    }
  ...
  Future<HealthSnapshot> fetchTodaySummary() async {
    ...
    try {
      final int? totalSteps = await _healthFactory!.getTotalStepsInInterval(start, now);
      steps = totalSteps ?? 0;
    ...
    try {
      final List<HealthDataPoint> points = await _healthFactory!
          .getHealthDataFromTypes(start, now, <HealthDataType>[HealthDataType.DISTANCE_WALKING_RUNNING]);
      final Iterable<HealthDataPoint> deduplicated = HealthDataPoint.removeDuplicates(points);
